...
const AdminDashboard = () => {
  ...
  const handleCreateProduct = async (e) => {
    ...
    try {
      ...
      showSuccess(`Product "${response.data.name}" created successfully ✅`);
  ...
  const handleUpdateProduct = async (e) => {
    ...
    try {
      ...
      showSuccess(`"${productToUpdate.name}" updated successfully`);
      return true; // ✅ for consistency
    } catch (err) {
      showError(err.response?.data?.error || "Error updating product");
      return false;
    }
  };
  ...
  const handleDeleteProduct = async (id) => {
    ...
    try {
      ...
      showError(err.response?.data?.error || "Error deleting product");
    return false; 
    }
  };
