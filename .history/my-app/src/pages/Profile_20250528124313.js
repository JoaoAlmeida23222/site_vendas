    ...
    const fetchUserData = async () => {
      try {
        ...
        if (response.ok) {
          ...
          setUser({
            ...data.user,
            createdAt: data.user.created_at // normalize field name
          });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (formData.avatarFile) {
        formDataToSend.append('avatar', formData.avatarFile); // must match backend field name
      }

      const profileRes = await fetch('/api/user/profile', {
        method: 'PUT',
        headers: {
          Authorization: `Bearer ${token}` // DON'T set Content-Type — browser will set it for multipart/form-data
        },
        body: formDataToSend
      });
    ...
    setUser({
      ...
      if (formData.avatarFile) {
        ...
        avatarForm.append('file', formData.avatarFile);

        const avatarRes = await fetch('/api/user/profile', {
          method: 'PUT',
          headers: { Authorization: `Bearer ${token}` },
          body: avatarForm
        });
