  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (formData.avatarFile) {
        formDataToSend.append('file', formData.avatarFile);
      }
      ...
      const data = await profileRes.json(); // ✅ Read once
      
      if (!profileRes.ok) {
        throw new Error(data.message || 'Failed to update profile');
      }
      
      // ✅ Use data below — this is now safe
      setUser(data.user);
      setIsEditing(false);
      
      // Toast logic
      if (formData.name !== user.name) showSuccess("Name updated successfully");
      if (formData.email !== user.email) showSuccess("Email updated successfully");
      if (formData.newPassword) showSuccess("Password updated successfully");
      if (formData.avatarFile) showSuccess("Avatar updated successfully");
