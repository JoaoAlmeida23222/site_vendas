...
const Login = ({ onLogin = () => {} }) => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (token) {
        ...
        // ✅ Only sync guest cart if backend cart is empty
  ...
  try {
    ...
    localStorage.setItem("token", token);
window.dispatchEvent(new Event("storage")); // ✅ ensures CartContext picks up the new token
localStorage.setItem("user", JSON.stringify(user)); // ✅ trigger reactive contexts like CartContext
