...
// 2. Fetch user's current backend cart
const backendCartRes = await fetch('http://localhost:5001/api/cart', {
  headers: {
...
// 3. Load guest cart from localStorage
const guestCart = JSON.parse(localStorage.getItem('guest_cart')) || [];

// 4. Only sync guest cart if user has no backend cart yet
if (guestCart.length > 0 && backendCart.length === 0) {
  ...
  // ✅ Clear guest cart immediately after sync
