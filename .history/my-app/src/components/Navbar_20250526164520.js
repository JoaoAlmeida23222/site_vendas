...
const Navbar = () => {
  ...
  const handleLogout = () => {
    clearCart(); // clears in-memory + guest cart
    localStorage.removeItem('token');
    localStorage.removeItem('user'); // optional, but good hygiene
    localStorage.removeItem('guest_cart'); // optional: if you want to reset guest cart too
    window.dispatchEvent(new Event('storage')); // 🔄 triggers token update in CartContext
    navigate('/');
  };
