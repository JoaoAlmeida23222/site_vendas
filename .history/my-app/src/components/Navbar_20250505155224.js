...
const Navbar = () => {
  ...
  }, [isLoggedIn, location.pathname, fetchCart]); // optional: refetch on route change
      ...
      <div className="nav-links">
        ...
        {cartItems.length > 0 ? (
          <Link to="/cart" className={`cart-link ${cartPulse ? 'pulse' : ''}`}>
            <FaShoppingCart />
            <span className="cart-count">{cartItems.reduce((sum, item) => sum + (item.quantity || 1), 0)}
            </span>
          </Link>
        ) : (
